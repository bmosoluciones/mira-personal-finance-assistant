        ...
        "operations": [
            {
                ...
                "transaction": {"amount": 100, "account_global_id": "missing", "type": "expense", "date": "2025-01-01"},
            }
        ]
    }
    res = service.push_transactions(token, payload)
...
def test_mobile_sync_category_conflicts(db):
    ...
    db.category.create("Cat1", "expense")
    db.category.create("Cat2", "expense")
...
def test_mobile_sync_transaction_update_and_delete(db):
    ...
    # 3. Successful delete
    del_payload = {
        "operations": [{"operation": "delete", "sync_id": tx["sync_id"], "base_version": updated_tx["sync_version"]}]
    }
...
def test_mobile_sync_unsupported_operation(db):
    ...
    payload = {"operations": [{"operation": "MAGIC", "sync_id": generate_ulid()}]}
    res = service.push_transactions(token, payload)
    assert res["results"][0]["status"] == "rejected"
